...
impl Grid {
    ...
    fn count_neighbours(&self, x: usize, y: usize) -> u8 {
        ...
        count += if x > 0 {
            ...
            partial_count += if y > 0 {
                if self.matrix[x - 1][y - 1] == 1 { 1 } else { 0 }
            } else {
                0
            };
            ...
            partial_count += if y < (self.width - 1) {
                if self.matrix[x - 1][y + 1] == 1 { 1 } else { 0 }
            } else {
                0
            };
        ...
        count += if y > 0 {
            ...
            partial_count += if x < (self.height - 1) {
                if self.matrix[x + 1][y - 1] == 1 { 1 } else { 0 }
            } else {
                0
            };
        ...
        count += if y < (self.width - 1) {
            if self.matrix[x][y + 1] == 1 { 1 } else { 0 }
        } else {
            0
        };
    ...
    pub fn clock(&mut self) {
        ...
        for (x, row) in new_matrix.iter_mut().enumerate().take(self.width) {
            for (y, field) in row.iter_mut().enumerate().take(self.width) {
                ...
                if ((neighbours_ammount == 2 || neighbours_ammount == 3) && *field == 1) ||
                    (neighbours_ammount == 3 && *field == 0)
                {
                    *field = 1;
                } else {
                    *field = 0;
...
mod tests {
    ...
    fn test_01_new_returns_a_valid_matrix() {
        // Create a matrix and it begins initialized with the right ammount of columns and
