...
fn draw_y_axis(grid_width: usize){
    ...
    draw_line(common_x1 + ARROW_WIDTH, FLOOR_Y + 10.0, common_x1 + 12.5, FLOOR_Y + 15.0, 2.0, BLACK);
    draw_line(common_x1 + ARROW_WIDTH, FLOOR_Y + 20.0, common_x1 + 12.5, FLOOR_Y + 15.0, 2.0, BLACK);
...
fn draw_x_axis(grid_height: usize){
    ...
    let common_y1 = FLOOR_Y + MATRIX_STEP_Y + (grid_height as f32)*20.0 + (grid_height as f32)*SQUARE_OFFSET;
    draw_line(FLOOR_X + 10.0, common_y1, FLOOR_X + 10.0, common_y1 + 12.5, 2.0, BLACK);
    draw_line(FLOOR_X + 5.0, common_y1 + ARROW_WIDTH, FLOOR_X + 10.0, common_y1 + 12.5, 2.0, BLACK);
    draw_line(FLOOR_X + 15.0, common_y1 + ARROW_WIDTH, FLOOR_X + 10.0, common_y1 + 12.5, 2.0, BLACK);
...
pub fn windows_logic(grid: &mut Grid, generation: &mut i32, auto_advance: &mut bool, chosen_cell: &mut (usize, usize), time_sleep: &mut f32) {
    let (grid_width, grid_height) = grid.dimensions();
    ...
    .ui(&mut root_ui(), |ui| {
        ...
        //ui.label(None, &format!("Chosen cell: {} {}", mouse_x, mouse_y));

        if is_mouse_button_pressed(MouseButton::Left) {
            let (mouse_x, mouse_y) = mouse_position();
            // Notar que los limites mayores de x e y dependen del tamaño de la tabla
        ...
        ui.label(None, &format!("Chosen cell: {} {}", chosen_cell.0, chosen_cell.1));
    });
}
...
async fn main() {
    ...
    loop {
        ...
        windows_logic(&mut grid, &mut generation, &mut auto_advance, &mut chosen_cell, &mut time_sleep);
